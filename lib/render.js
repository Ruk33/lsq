...
function command(name = "", props = {}) {
    ...
    if (state.debug) {
        console.log("%c      COMMAND ", "background:rgb(180, 122, 41);", { name, command: { name, props }, state_before_command: JSON.parse(prev_version), state_after_command: state, })
        // console.log("%c      COMMAND ", "background:rgb(180, 122, 41);", { name, props })
...
function request_navigation(path = "") {
    window.command("navigate", path)
}
